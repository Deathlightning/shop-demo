...
<div class="wrapper">
    <div class="nav">
        <c:forEach items="${requestScope.classifyList}" var="item">
            <div class="nav-title" onclick="findByType('${item}')">${item}</div>
        </c:forEach>
    </div>
        ...
        <div class="row">
            <c:forEach items="${requestScope.goodList}" var="item">
                <div class="col-md-3 good-box">
                    ...
                    <button style="margin-left: 80px;" class="btn btn-info" onclick="insertShopCart(${item.id})">管理&nbsp;</button>
                    <button style="margin-left: 80px;" class="btn btn-info" onclick="insertShopCart(${item.id})">下架
                    </button>
                </div>
            </c:forEach>
        </div>
...
<script>
    ...
    function findByType(classify) {
        window.location.href = '${pageContext.request.contextPath}/good/listOnTypeController?classify=' + classify;
    }
</script>
</html>